...
class BrowserListeners {
  ...
  private onAttachedListener = (number: number, info: chrome.tabs.TabAttachInfo) =>
    this.onAttached(number, info)
  private onDetachedListener = (number: number, info: chrome.tabs.TabDetachInfo) =>
    this.onDetached(number, info)
          ...
          if (existing) {
            ...
            useContentService()
              .saveContent(
                ...
                chromeTab.url,
                [...tokens].join(' '),
                contentRequest['metas' as keyof object],
                chromeTab.title || '',
                [],
              )
          ...
          if (hostname.indexOf(e.hostnameMatch) >= 0) {
            ...
            switch (e.type) {
              ...
              case ExtractorType.HTML_SELECTOR:
                ...
                if (e.selector) {
                  const selection = doc.querySelector(e.selector)
  ...
  private async handleCaptureAnnotation(
    request: any,
    sender: chrome.runtime.MessageSender,
    sendResponse: any,
  ) {
  ...
  private async handleMessageWebsiteImage(
    request: any,
    sender: chrome.runtime.MessageSender,
    sendResponse: any,
  ) {
